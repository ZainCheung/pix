...
export function llmsTxt() {
  ...
  const lines = [
    ...
    '',
    '## Product',
    `- [Homepage](${siteUrl('/')}) — Product overview and the two-device setup model.`,
    `- [Set up Pix](${siteUrl('/start')}) — Install Pix on your computer and iPhone, then pair them once.`,
    `- [Join Pix iOS TestFlight](${IOS_APP_URL}) — Pix for iPhone, public beta via TestFlight.`,
